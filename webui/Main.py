...
from app.services import cache_manager, llm, video, voice, webui_task, semantic_director
...
def _render_brand(available_update: str | None = None):
    ...
    st.markdown(
        f"""
        <h1 class="mpt-brand">
            <span class="mpt-brand__name" style="background: linear-gradient(135deg, #00f2fe 0%, #4facfe 100%); -webkit-background-clip: text; -webkit-text-fill-color: transparent; font-weight: 800;">VideoPro Creative Studio</span>
            <span style="font-size: 0.45em; opacity: 0.8; font-weight: 500; margin-left: 12px; color: #a0aec0;">AI Video Director & Multi-Layer Studio</span>
            <a class="mpt-brand__version"
               href="https://143-47-35-167.sslip.io/pro/videopro/"
               target="_blank"
               rel="noopener noreferrer"
               aria-label="VideoPro"
               title="VideoPro Creative Studio">v2.5.0-PRO</a>
            {update_link}
        </h1>
        """,
...
def _render_settings_dialog():
    with st.container():
        st.markdown("""
        <div style="background: rgba(15, 23, 42, 0.9); border: 1px solid rgba(255, 255, 255, 0.12); border-radius: 12px; padding: 14px 18px; margin-bottom: 16px;">
            <div style="font-size: 15px; font-weight: 700; color: #f8fafc; margin-bottom: 6px;">📋 Matriz de Funcionamiento e Infraestructura</div>
            <div style="font-size: 12px; color: #94a3b8; line-height: 1.5;">
                • <b>FLUX 3 & LTX-2.5:</b> Mismo sistema visual, ejecutable en <b>GPU Gratis (ZeroGPU)</b> o <b>Pago (Replicate H100)</b>. Audio/diálogos de personajes integrados.<br>
                • <b>Google Flow:</b> Siempre vía <b>Navegador Web (Playwright)</b>. Vídeo cinemático 4K.<br>
                • <b>Google Flow Music:</b> Siempre vía <b>Navegador Web (Playwright)</b>. Genera la música máster.<br>
                • <b>Kokoro HD:</b> <b>CPU Local ($0)</b>. Solo si se activa locutor en off.<br>
                • <b>Subtítulos & Música:</b> Se pueden desactivar completamente eligiendo <i>'Sin Subtítulos'</i> y <i>'Sin Música'</i>.
            </div>
        </div>
        """, unsafe_allow_html=True)
        _set_runtime_config("app", "hide_config", False)

        tab_video, tab_voice, tab_llm, tab_stock, tab_subtitles, tab_storage = st.tabs([
            "🛸 Vídeo",
            "🎙️ Voz & Audio",
            "🧠 LLMs",
            "🎞️ Materiales",
        ...
        # 1. TAB VÍDEO (LTX-2.5, FLUX 3 & Replicate)
        with tab_video:
            st.subheader("Motores de Vídeo: LTX-2.5 & FLUX 3")
            st.caption("Generación en clústeres GPU gratuitos ZeroGPU o en clústeres dedicados H100.")

            # Hugging Face Pool
            hf_tokens_list = config.serverless_pool.get("hf_tokens", []) if hasattr(config, "serverless_pool") else []
            if not hf_tokens_list:
                hf_tokens_list = ["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", "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"]

            st.markdown("**1. Hugging Face ZeroGPU Pool (LTX-2.5 & FLUX 3 - Gratuito)**")
            st.caption("Tokens activos: " + str(len(hf_tokens_list)) + " | 🔗 [Obtener Token Hugging Face Gratis](https://huggingface.co/settings/tokens) *(Permiso 'Read')*")
            
            c_tok1, c_tok2 = st.columns([3, 1])
            with c_tok1:
                new_hf = st.text_input("Añadir Token HF", type="password", key="clean_hf_tok", placeholder="hf_xxxxxxxxxxxxxxxxxxxx", label_visibility="collapsed")
            with c_tok2:
                if st.button("➕ Añadir", key="clean_btn_add_hf", use_container_width=True):
                    if new_hf.strip() and new_hf.strip() not in hf_tokens_list:
                        hf_tokens_list.append(new_hf.strip())
                        st.toast("Token añadido al pool")
                        st.rerun()

            st.divider()
            st.markdown("**2. Replicate Pro Cloud (Pago / Inferencia Instantánea en H100)**")
            st.caption("Renderizado sin colas en H100s | 🔗 [Obtener Token en Replicate](https://replicate.com/account/api-tokens) *(Empieza por r8_...)*")
            cur_rep = config.app.get("replicate_api_token", "")
            new_rep = st.text_input("Replicate API Token", value=cur_rep, type="password", key="clean_rep_tok", placeholder="r8_xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx")
            _set_runtime_config("app", "replicate_api_token", new_rep)

            use_rep = st.checkbox("Priorizar Replicate sobre ZeroGPU para máxima velocidad", value=config.app.get("use_replicate_priority", False), key="clean_chk_rep")
            _set_runtime_config("app", "use_replicate_priority", use_rep)

        # 2. TAB VOZ & AUDIO (Kokoro HD, VibeVoice, Flow Music, ElevenLabs, Fish Audio)
        with tab_voice:
            st.subheader("Síntesis de Voz & Música")
            st.caption("Locución local en español (Kokoro HD 24kHz), inglés (VibeVoice) y música con Flow Music.")

            st.markdown("**1. Proveedor de Locución**")
            st.caption("Kokoro HD (Local sin coste) | VibeVoice (ZeroGPU) | 🔗 [Obtener Key ElevenLabs](https://elevenlabs.io) | 🔗 [Obtener Key Fish Audio](https://fish.audio/go-api/)")
            voice_prov = st.selectbox(
                "Motor de Voz",
                ["Kokoro HD (Local Gratuito 24kHz)", "VibeVoice (ZeroGPU A100)", "Edge TTS (Microsoft Cloud Gratuito)", "ElevenLabs (API de Pago)", "Fish Audio (Clonación Zero-Shot)"],
                index=0,
                key="clean_voice_prov"
            )

            c_v1, c_v2 = st.columns(2)
            with c_v1:
                el_key = config.app.get("elevenlabs_api_key", "")
                new_el = st.text_input("ElevenLabs API Key", value=el_key, type="password", key="clean_el_key", placeholder="sk_xxxxxxxxxxxxxxxxxxxx")
                _set_runtime_config("app", "elevenlabs_api_key", new_el)
            with c_v2:
                fish_k = config.app.get("fish_audio_api_key", "")
                new_fish = st.text_input("Fish Audio API Key", value=fish_k, type="password", key="clean_fish_k", placeholder="fa_xxxxxxxxxxxxxxxxxxxx")
                _set_runtime_config("app", "fish_audio_api_key", new_fish)

            st.divider()
            st.markdown("**2. Google Flow Music (Lyria 3 Pro)**")
            st.caption("Composición musical multi-minuto y bandas sonoras a compás | 🔗 [Abrir Google Flow Music](https://flowmusic.app/)")
            fm_tok = config.app.get("flowmusic_session", "")
            new_fm = st.text_input("Flow Music Session Token / Cookie", value=fm_tok, type="password", key="clean_fm_tok", placeholder="session_xxxxxxxxxxxxxxxxxxxx")
            _set_runtime_config("app", "flowmusic_session", new_fm)

        # 3. TAB LLMS (Gemini, Groq, DeepSeek, OpenRouter, Together, Cerebras)
        with tab_llm:
            st.subheader("Modelos de Lenguaje & Guionización")
            st.caption("Generación de guiones cinematográficos y escaletas DoP de 4 y 7 capas.")

            llm_prov = st.selectbox(
                "Proveedor LLM Principal",
                ["Antigravity Bridge (Gemini 3.7 Flash High - Local)", "Google AI Studio (Gemini 2.5 / 3.7)", "Groq Cloud (Llama 3.3 / DeepSeek R1)", "OpenRouter", "DeepSeek Directo", "Together AI", "Cerebras Cloud"],
                index=0,
                key="clean_llm_prov"
            )
            ...
            with c_k1:
                st.markdown("🔗 [Google AI Studio API Key](https://aistudio.google.com/app/apikey)")
                gem_k = config.app.get("gemini_api_key", "")
                new_gem = st.text_input("Gemini API Key", value=gem_k, type="password", key="clean_gem_k", label_visibility="collapsed", placeholder="AIzaSyxxxxxxxxxxxxxxxxxxxx")
                _set_runtime_config("app", "gemini_api_key", new_gem)
                
                st.markdown("🔗 [OpenRouter Keys](https://openrouter.ai/keys)")
                or_k = config.app.get("openrouter_api_key", "")
                new_or = st.text_input("OpenRouter Key", value=or_k, type="password", key="clean_or_k", label_visibility="collapsed", placeholder="sk-or-v1-xxxxxxxxxxxxxxxxxxxx")
                _set_runtime_config("app", "openrouter_api_key", new_or)

                st.markdown("🔗 [Together AI Keys](https://api.together.xyz/settings/api-keys)")
                tog_k = config.app.get("together_api_key", "")
                new_tog = st.text_input("Together Key", value=tog_k, type="password", key="clean_tog_k", label_visibility="collapsed", placeholder="tog_xxxxxxxxxxxxxxxxxxxx")
                _set_runtime_config("app", "together_api_key", new_tog)

            with c_k2:
                st.markdown("🔗 [Groq Cloud Console Keys](https://console.groq.com/keys)")
                groq_k = config.app.get("groq_api_key", "")
                new_groq = st.text_input("Groq API Key", value=groq_k, type="password", key="clean_groq_k", label_visibility="collapsed", placeholder="gsk_xxxxxxxxxxxxxxxxxxxx")
                _set_runtime_config("app", "groq_api_key", new_groq)

                st.markdown("🔗 [DeepSeek Platform Keys](https://platform.deepseek.com/api_keys)")
                ds_k = config.app.get("deepseek_api_key", "")
                new_ds = st.text_input("DeepSeek Key", value=ds_k, type="password", key="clean_ds_k", label_visibility="collapsed", placeholder="sk-xxxxxxxxxxxxxxxxxxxx")
                _set_runtime_config("app", "deepseek_api_key", new_ds)

                st.markdown("🔗 [Cerebras Cloud Console](https://cloud.cerebras.ai/)")
                cer_k = config.app.get("cerebras_api_key", "")
                new_cer = st.text_input("Cerebras Key", value=cer_k, type="password", key="clean_cer_k", label_visibility="collapsed", placeholder="csk-xxxxxxxxxxxxxxxxxxxx")
                _set_runtime_config("app", "cerebras_api_key", new_cer)

        # 4. TAB MATERIALES & STOCK
        with tab_stock:
            st.subheader("Materiales de Archivo & Stock HD")
            st.caption("Descarga de metraje y fotos de archivo de alta definición.")

            c_st1, c_st2 = st.columns(2)
            with c_st1:
                st.markdown("🔗 [Obtener Pexels API Key Gratis](https://www.pexels.com/api/)")
                pex_k = config.app.get("pexels_api_key", "")
                new_pex = st.text_input("Pexels Key", value=pex_k, type="password", key="clean_pex_k", label_visibility="collapsed", placeholder="pexels_key_xxxxxxxxxxxxxxxxxxxx")
                _set_runtime_config("app", "pexels_api_key", new_pex)
            with c_st2:
                st.markdown("🔗 [Obtener Pixabay API Key Gratis](https://pixabay.com/api/docs/)")
                pix_k = config.app.get("pixabay_api_key", "")
                new_pix = st.text_input("Pixabay Key", value=pix_k, type="password", key="clean_pix_k", label_visibility="collapsed", placeholder="pixabay_key_xxxxxxxxxxxxxxxxxxxx")
                _set_runtime_config("app", "pixabay_api_key", new_pix)

        # 5. TAB SUBTÍTULOS & TRANSCRIPCIÓN SOTA
        with tab_subtitles:
            st.subheader("Subtítulos & Transcripción SOTA")
            st.caption("Alineación fonética milimétrica y motores de transcripción de ultra-alta velocidad.")

            st.markdown("**1. Motor de Transcripción Principal**")
            st.caption("Faster-Whisper (Local CPU) | 🔗 [Groq Cloud (Whisper Large v3 Turbo)](https://console.groq.com/keys) | 🔗 [Deepgram Nova-2](https://console.deepgram.com/) | 🔗 [TwelveLabs API](https://twelvelabs.io)")
            
            whisper_mode = st.selectbox(
                "Motor de Transcripción",
                [
                    "Groq Whisper Large v3 Turbo (Cloud Ultra-Rápido 300x)",
                    "Faster-Whisper CTranslate2 (Local CPU/GPU sin coste)",
                    "Deepgram Nova-2 (Milimétrico para TikTok/Shorts)",
                    "TwelveLabs Pegasus (Comprensión Semántica de Vídeo)",
                    "WhisperX (Forced Alignment Fonético)"
                ],
                index=0,
                key="clean_whisper_engine"
            )

            c_tr1, c_tr2 = st.columns(2)
            with c_tr1:
                st.markdown("🔗 [Deepgram API Key (Créditos Gratis)](https://console.deepgram.com/)")
                dg_k = config.app.get("deepgram_api_key", "")
                new_dg = st.text_input("Deepgram Key", value=dg_k, type="password", key="clean_dg_k", label_visibility="collapsed", placeholder="dg_xxxxxxxxxxxxxxxxxxxx")
                _set_runtime_config("app", "deepgram_api_key", new_dg)
            with c_tr2:
                st.markdown("🔗 [TwelveLabs API Key (10h Gratis)](https://twelvelabs.io)")
                tl_k = config.app.get("twelvelabs_api_key", "")
                new_tl = st.text_input("TwelveLabs Key", value=tl_k, type="password", key="clean_tl_k", label_visibility="collapsed", placeholder="tl_xxxxxxxxxxxxxxxxxxxx")
                _set_runtime_config("app", "twelvelabs_api_key", new_tl)

        # 6. TAB ALMACENAMIENTO & SISTEMA
        with tab_storage:
...
def _render_script_settings(panel, params):
    ...
    with panel:
        with st.container(border=True):
            ...
            # Director Semántico & Presets Maestros
            style_presets = [
                ("autoflow_city", "🏛️ Autoflow City & Dron 8K (ARRI Alexa 65, Golden Hour)"),
                ("vox_documentary", "📜 Vox / Johnny Harris Documental (Papel táctil, Diorama 2.5D)"),
                ("pixar_3d", "🧸 Pixar / 3D Stylized (Octane 3D, Iluminación Cálida)"),
                ("bloomberg_fintech", "📈 Bloomberg / FinTech & Geopolítica (Dark Glassmorphism)"),
                ("standard", "🎬 Estándar / Balanceado General"),
            ]
            selected_style = stable_selectbox(
                "🎨 Estilo Visual y Dirección de Arte",
                options=[val for val, _ in style_presets],
                default_value="standard",
                key="director_style_preset_select",
                format_func=lambda val: dict(style_presets)[val],
            )
            
            params.video_subject = st.text_area(
                tr("Video Subject"),
                placeholder="Ejemplo: La evolución de los rascacielos de Madrid y el nuevo skyline...",
                height=96,
                key="video_subject",
            ).strip()
...
def _render_audio_settings(panel, params):
    ...
    with panel:
        with st.container(border=True):
            ...
            elif selected_tts_server == "vibevoice":
                filtered_voices = voice.get_vibevoice_voices()
            ...
            def _friendly(v):
                ...
                if voice.is_vibevoice_voice(v):
                    return v.replace("vibevoice:", "")
...
def _process_director_input(user_text: str, params):
    """Procesa una entrada del usuario en el chat del Director Creativo."""
    st.session_state["director_messages"].append({"role": "user", "content": user_text})
    
    with st.spinner("El Director Creativo está analizando tu propuesta..."):
        res = semantic_director.chat_with_director(
            messages=st.session_state["director_messages"][:-1],
            user_input=user_text
        )
    
    st.session_state["director_messages"].append({
        "role": "assistant",
        "content": res["response_text"],
        "suggestions": res["suggestions"]
    })
    st.session_state["director_spec"] = res["spec"]
    
    spec = res["spec"]
    if spec.get("subject"):
        params.video_subject = spec["subject"]
        st.session_state["video_subject"] = spec["subject"]
    if spec.get("recommended_source"):
        params.video_source = spec["recommended_source"]
        _set_runtime_config("app", "video_source", spec["recommended_source"])
    if spec.get("voice_preset"):
        params.voice_name = spec["voice_preset"]
        _set_runtime_config("ui", "voice_name", spec["voice_preset"])
...
• **Audio:** Flow Music (Lyria 3) 🟢
• **LLM:** Gemini 3.7 Flash High 🟢
        """)
...
def _render_application():
    ...
    # 1. Estudio de Co-Creación y Chat Semántico del Director
    ...
    uploaded_audio_file = uploaded_audio_file if 'uploaded_audio_file' in locals() else None
    uploaded_bgm_file = uploaded_bgm_file if 'uploaded_bgm_file' in locals() else None
    voice_mode = voice_mode if 'voice_mode' in locals() else VOICE_MODE_TTS
