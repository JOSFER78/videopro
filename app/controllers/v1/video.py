...
@router.get("/voices", summary="Get list of available voices including VibeVoice 1.5B and EdgeTTS")
def get_voices():
    azure_voices = voice.get_all_azure_voices()
    return utils.get_response(200, {"voices": azure_voices})


@router.post("/voice/preview", summary="Generate a quick audio preview of a voice")
def generate_voice_preview(request: Request, body: dict):
    text = body.get("text", "¡Hola! Este es nuestro estudio de vídeo autónomo VideoPro con VibeVoice.")
    voice_name = body.get("voice_name", "vibevoice:es-emilio-Male")
    voice_rate = float(body.get("voice_rate", 1.0))
    voice_volume = float(body.get("voice_volume", 1.0))
    preview_id = utils.get_uuid()
    output_dir = utils.storage_dir("voice_previews", create=True)
    out_file = os.path.join(output_dir, f"preview_{preview_id}.mp3")
    ...
    if not os.path.exists(out_file):
        raise HttpException(task_id=preview_id, status_code=500, message="Failed to generate voice preview")

    return utils.get_response(200, {
        "preview_id": preview_id,
        "file": f"voice_previews/preview_{preview_id}.mp3",
        "url": f"/stream/voice_previews/preview_{preview_id}.mp3"
    })


@router.post("/scene/generate", summary="Generate a single scene clip using Stock, FLUX Keyframe, or Google Flow")
def generate_single_scene(request: Request, body: dict):
    scene_id = body.get("scene_id", 1)
    mode = body.get("mode", "flux")  # "stock", "flux", "google_flow"
    prompt = body.get("prompt", "Cinematic 35mm documentary scene")
    image_ref = body.get("image_reference", "")
    aspect = body.get("aspect", "9:16")
    duration = float(body.get("duration", 5.0))

    output_dir = utils.storage_dir("scene_previews", create=True)
    clip_id = utils.get_uuid()
    out_clip = os.path.join(output_dir, f"scene_{scene_id}_{clip_id}_{mode}.mp4")

    if mode == "flux":
        from app.services.flux_keyframe import generate_flux_keyframe, synthesize_flux_clip
        temp_img = out_clip.replace(".mp4", "_keyframe.png")
        generate_flux_keyframe(prompt, temp_img, aspect_ratio=aspect, reference_image=image_ref)
        synthesize_flux_clip(temp_img, out_clip, duration=duration, aspect_ratio=aspect)
    elif mode == "google_flow":
        from app.services.google_flow_runner import generate_google_flow_clip_sync
        generate_google_flow_clip_sync(prompt, out_clip, image_reference=image_ref, timeout_seconds=180)
    else:  # stock
        from app.services.flux_keyframe import generate_flux_keyframe, synthesize_flux_clip
        temp_img = out_clip.replace(".mp4", "_keyframe.png")
        generate_flux_keyframe(prompt, temp_img, aspect_ratio=aspect)
        synthesize_flux_clip(temp_img, out_clip, duration=duration, aspect_ratio=aspect)

    return utils.get_response(200, {
        "scene_id": scene_id,
        "mode": mode,
        "clip_file": f"scene_previews/scene_{scene_id}_{clip_id}_{mode}.mp4",
        "url": f"/stream/scene_previews/scene_{scene_id}_{clip_id}_{mode}.mp4"
    })
