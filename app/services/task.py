...
from app.services import real_news_image_service
...
def generate_audio(task_id, params, video_script, voice_preview=None):
    ...
    if not custom_audio_file:
        # Check if user explicitly requested NO voiceover / native scene audio
        if str(params.voice_name).lower() in ["none", "no_voice", "silent", "none_dialogue", ""] or getattr(params, "voice_volume", 1.0) == 0:
            logger.info("Voiceover disabled by user (native video dialogue mode / silent voiceover).")
            # Return no voiceover audio file, duration from paragraphs or video clips
            return None, 15, None
...
def get_video_materials(task_id, params, video_terms, audio_duration):
    # Soporte para los 3 modos de generación por escena (Stock DB, FLUX.1 Keyframe 0, Google Flow Playwright)
    if getattr(params, "scenes", None) and len(params.scenes) > 0:
        logger.info(f"\n\n## processing {len(params.scenes)} hybrid scenes (Stock / FLUX / Google Flow)")
        scene_clips = []
        task_folder = utils.task_dir(task_id)
        aspect_val = "9:16" if getattr(params.video_aspect, "value", str(params.video_aspect)) in ("9:16", "portrait") else "16:9"

        from app.services.flux_keyframe import generate_flux_keyframe, synthesize_flux_clip
        from app.services.google_flow_runner import generate_google_flow_clip_sync

        for idx, sc in enumerate(params.scenes):
            sc_mode = getattr(sc, "mode", "stock") or "stock"
            sc_prompt = getattr(sc, "prompt", "") or getattr(sc, "text", "") or f"Scene {idx+1}"
            sc_ref = getattr(sc, "image_reference", "") or ""
            sc_duration = float(getattr(sc, "duration", 0) or params.video_clip_duration or 5.0)
            sc_out_clip = path.join(task_folder, f"scene_{idx+1}_{sc_mode}.mp4")

            logger.info(f"Rendering scene {idx+1}/{len(params.scenes)} | Mode: {sc_mode} | Prompt: {sc_prompt[:60]}...")

            if sc_mode in ("real_news", "real_photo"):
                real_img = path.join(task_folder, f"scene_{idx+1}_real_news.jpg")
                res_photo = real_news_image_service.fetch_real_photo_for_scene(sc_prompt, real_img)
                if res_photo and path.exists(real_img):
                    clip_path = synthesize_flux_clip(real_img, sc_out_clip, duration=sc_duration, aspect_ratio=aspect_val)
                    scene_clips.append(clip_path)
                else:
                    keyframe_img = path.join(task_folder, f"scene_{idx+1}_fallback_flux.png")
                    generate_flux_keyframe(sc_prompt, keyframe_img, aspect_ratio=aspect_val)
                    clip_path = synthesize_flux_clip(keyframe_img, sc_out_clip, duration=sc_duration, aspect_ratio=aspect_val)
                    scene_clips.append(clip_path)
            elif sc_mode == "flux":
                keyframe_img = path.join(task_folder, f"scene_{idx+1}_flux_keyframe.png")
                generate_flux_keyframe(sc_prompt, keyframe_img, aspect_ratio=aspect_val, reference_image=sc_ref)
                clip_path = synthesize_flux_clip(keyframe_img, sc_out_clip, duration=sc_duration, aspect_ratio=aspect_val)
                scene_clips.append(clip_path)
            elif sc_mode == "google_flow":
                res = generate_google_flow_clip_sync(
                    prompt=sc_prompt,
                    output_mp4_path=sc_out_clip,
                    image_reference=sc_ref,
                    timeout_seconds=180
                )
                scene_clips.append(res["clip_path"])
            else:  # stock mode
                stock_res = material.download_videos(
                    task_id=task_id,
                    search_terms=sc_prompt,
                    source=params.video_source or "pexels",
                    video_aspect=params.video_aspect,
                    video_concat_mode=VideoConcatMode.sequential,
                    audio_duration=sc_duration,
                    max_clip_duration=params.video_clip_duration,
                    match_script_order=True,
                )
                if stock_res and len(stock_res) > 0:
                    scene_clips.append(stock_res[0])
                else:
                    real_img = path.join(task_folder, f"scene_{idx+1}_real_fallback.jpg")
                    res_photo = real_news_image_service.fetch_real_photo_for_scene(sc_prompt, real_img)
                    if res_photo and path.exists(real_img):
                        clip_path = synthesize_flux_clip(real_img, sc_out_clip, duration=sc_duration, aspect_ratio=aspect_val)
                        scene_clips.append(clip_path)
                    else:
                        keyframe_img = path.join(task_folder, f"scene_{idx+1}_stock_fallback.png")
                        generate_flux_keyframe(sc_prompt, keyframe_img, aspect_ratio=aspect_val)
                        clip_path = synthesize_flux_clip(keyframe_img, sc_out_clip, duration=sc_duration, aspect_ratio=aspect_val)
                        scene_clips.append(clip_path)
                    scene_clips.append(clip_path)
        ...
        if scene_clips:
            logger.success(f"Generated {len(scene_clips)} scene clips for hybrid workflow")
            return scene_clips
