...
_VIBEVOICE_THREAD_LOCK = threading.Lock()
VIBEVOICE_LOCK_FILE = "/tmp/vibevoice_synthesis.lock"
...
def get_vibevoice_voices() -> list[str]:
    """Retorna las voces de VibeVoice 1.5B (motor local a coste cero y alta expresividad)."""
    return [
        "vibevoice:es-emilio-Male (VibeVoice 1.5B Español - Emoción/Cinemático)",
        "vibevoice:en-Carter-Male (VibeVoice 1.5B English Narrator)",
        "vibevoice:en-Frank-Male (VibeVoice 1.5B English Deep)",
        "vibevoice:en-Alice-Female (VibeVoice 1.5B English Calm)",
        "vibevoice:zh-Bowen-Male (VibeVoice 1.5B Chinese)",
    ]


def is_vibevoice_voice(voice_name: str | None) -> bool:
    """Verifica si la voz solicitada corresponde al motor local VibeVoice."""
    if not voice_name:
        return False
    v_lower = str(voice_name).lower()
    return "vibevoice" in v_lower or "es-emilio" in v_lower
...
def get_all_azure_voices(filter_locals=None) -> list[str]:
    voices = []
    # Incorporar VibeVoice como motor prioritario en el catálogo de voces
    for vv in get_vibevoice_voices():
        voices.append(vv)
...
def tts(
    ...
    if is_vibevoice_voice(voice_name):
...
def chatterbox_tts(
    ...
    payload = {
        ...
        "voice": voice,
        "response_format": "mp3",
        # OpenAI speech API accepts speed 0.25-4.0; VideoPro Studio's rate is a
...
def vibevoice_tts(
    ...
    """
    Sintetiza locución hiperrealista con el motor local VibeVoice 1.5B.
    Utiliza /home/ubuntu/vibevoice-venv/bin/python y la voz en español 'es-emilio'.
    """
    text = (text or "").strip()
    if not text:
        logger.error("VibeVoice TTS text is empty")
        return None

    ensure_file_path_exists(voice_file)
    output_dir = os.path.dirname(os.path.abspath(voice_file))

    # Extraer speaker name
    speaker = "es-emilio"
    v_clean = voice_name.lower().replace("vibevoice:", "").split("-")[0].strip()
    if "carter" in v_clean:
        speaker = "en-Carter"
    elif "frank" in v_clean:
        speaker = "en-Frank"
    elif "alice" in v_clean:
        speaker = "en-Alice"
    elif "bowen" in v_clean:
        speaker = "zh-Bowen"
    elif "emilio" in v_clean or not v_clean:
        speaker = "es-emilio"

    # Archivo temporal con formato 'Speaker 1: <texto>'
    tmp_txt_path = os.path.join(output_dir, f"vibe_{os.getpid()}_{int(time.time()*1000)}.txt")
    with open(tmp_txt_path, "w", encoding="utf-8") as f:
        f.write(f"Speaker 1: {text}\n")

    python_exec = "/home/ubuntu/vibevoice-venv/bin/python"
    if not os.path.exists(python_exec):
        python_exec = "python3"

    model_dir = "/home/ubuntu/models/VibeVoice-1.5B"
    if not os.path.exists(model_dir):
        model_dir = "microsoft/VibeVoice-1.5b"

    script_path = "/home/ubuntu/VibeVoice/demo/inference_from_file.py"
    ...
    logger.info(f"Executing VibeVoice 1.5B TTS (Speaker: {speaker}, CFG: {cfg_scale}) with process lock...")
    try:
        with _VIBEVOICE_THREAD_LOCK:
            with open(VIBEVOICE_LOCK_FILE, "w") as lock_f:
                ...
                try:
                    ...
                    try:
                        ...
                        pass
        stem = Path(tmp_txt_path).stem
        gen_wav = os.path.join(output_dir, f"{stem}_generated.wav")

        if not os.path.exists(gen_wav) or os.path.getsize(gen_wav) < 1000:
            logger.warning(f"VibeVoice output wav not found, fallback to EdgeTTS for safety: {res.stderr[:200]}")
            fallback_voice = "es-ES-ElviraNeural-Female" if "female" in voice_name.lower() else "es-ES-AlvaroNeural-Male"
            return azure_tts_v1(text, fallback_voice, voice_rate, voice_file, voice_volume)

        # Transcodificar al formato destino y ajustar volumen/tempo
        ffmpeg_bin = utils.get_ffmpeg_binary()
        vol_filter = f"volume={voice_volume}" if voice_volume != 1.0 else "anull"
        tempo_filter = f",atempo={voice_rate}" if voice_rate != 1.0 else ""
        af_chain = f"{vol_filter}{tempo_filter}"
        ...
        if os.path.exists(gen_wav) and gen_wav != voice_file:
            try:
                os.remove(gen_wav)
            except Exception:
                pass
        ...
        logger.success(f"VibeVoice TTS completed: {voice_file} ({audio_duration:.2f}s)")
        sub_maker = ensure_legacy_submaker_fields(SubMaker())
        return populate_legacy_submaker_with_full_text(
            sub_maker=sub_maker,
            text=text,
        ...
        fallback_voice = "es-ES-AlvaroNeural-Male"
        return azure_tts_v1(text, fallback_voice, voice_rate, voice_file, voice_volume)
    finally:
