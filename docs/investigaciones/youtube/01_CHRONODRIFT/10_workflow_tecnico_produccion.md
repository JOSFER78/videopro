...
│   ├── 10_workflow_tecnico_produccion.md   # Este documento técnico normativo
│   └── 07_escaleta_10_primeros_episodios.md # Escaletas detalladas de producción
├── scripts/
...
│   ├── render_remotion.py                  # Composición Remotion 4.x, HUD y masterización EBU R128
│   └── verify-assets.py                    # QA Gates automatizados (ffprobe, cv2, ebur128)
├── data/
│   ├── grounding_sources/                  # Panoramas 360°, cubemaps y 6 perspectivas extraídas
...
Cada localización geográfica se descompone en 6 cámaras sintéticas proyectadas a partir de panoramas esféricos equirectangulares de alta resolución ($7680 \times 3840$ px, Master 8K):

| Identificador | Nombre de Perspectiva | Heading ($\alpha$) | Pitch ($\beta$) | Roll ($\gamma$) | FoV | Función Fotogramétrica y Rol en Matriz |
| :--- | :--- | :---: | :---: | :---: | :---: | :--- |
| **`CAM_N`** | Frontal Axial (Norte) | $0.0^\circ$ | $0.0^\circ$ | $0.0^\circ$ | $90^\circ$ | Vector de referencia axial y fachada frontal principal. |
| **`CAM_E`** | Flanco Lateral Este | $90.0^\circ$ | $0.0^\circ$ | $0.0^\circ$ | $90^\circ$ | Perspectiva lateral derecha, paralaje de horizonte y fuga urbana. |
| **`CAM_S`** | Eje de Retirada (Sur) | $180.0^\circ$ | $0.0^\circ$ | $0.0^\circ$ | $90^\circ$ | Retro-perspectiva y anclaje de fuga trasera para movimientos reversos. |
| **`CAM_W`** | Flanco Lateral Oeste | $270.0^\circ$ | $0.0^\circ$ | $0.0^\circ$ | $90^\circ$ | Perspectiva lateral izquierda y cálculo de sombras proyectadas. |
| **`CAM_PITCH_DOWN`** | Inmersión Cenital (Picado) | Vector de Avance | $-20.0^\circ$ | $0.0^\circ$ | $100^\circ$ | Textura de calzada, cimientos, escala peatonal y relieve basal. |
| **`CAM_PITCH_UP`** | Elevación Skyline (Contrapicado) | Vector de Avance | $+25.0^\circ$ | $0.0^\circ$ | $100^\circ$ | Cúspide de rascacielos, cañón urbano y cielo abierto. |

### 2.2 Descomposición Equirectangular a Cubemap 6 Caras

El panorama equirectangular $P(\phi, \theta)$ se transforma en 6 caras de cubo planas mediante la transformación de coordenadas esféricas a cartesianas:

$$x = \cos(\theta)\cos(\phi), \quad y = \sin(\theta), \quad z = \cos(\theta)\sin(\phi)$$

Cada cara del cubemap se muestrea con interpolación bicúbica a resolución $3840 \times 2160$ px para garantizar nitidez cristalina en los planos de detalle.
...
1. **`building:levels` y `height`:** Altura métrica real de cada inmueble. Si `height` no está tipificado, se interpola a razón de $3.5\text{ m}$ por planta.
...
3. **Copernicus DEM 30m:** Elevación sobre el nivel del mar ($Z_{\text{ground}}$) para calcular la altitud barométrica relativa real ($AGL = Z_{\text{dron}} - Z_{\text{ground}}$).
...
$$\text{Var}(\nabla^2 I) = \frac{1}{N} \sum_{x,y} \left( \nabla^2 I(x,y) - \mu_{\nabla^2} \right)^2 \ge 100.0$$

- **Resolución Mínima:** $3840 \times 2160$ píxeles (4K nativo).
...
### 2.5 Script Operativo: `streetview_multitemporal_scraper.py`

```python
#!/usr/bin/env python3
"""
streetview_multitemporal_scraper.py
Scraper multi-temporal de Street View 6-DoF + Ingesta OpenStreetMap + Filtro Laplaciano.
"""
import os
import sys
import json
...
CONFIG_PATH = WORKSPACE / "docs/investigaciones/youtube/01_CHRONODRIFT/channel_config.json"
OUTPUT_DIR = WORKSPACE / "data/grounding_sources"

def load_channel_config():
    with open(CONFIG_PATH, "r", encoding="utf-8") as f:
        return json.load(f)

def validate_laplacian_sharpness(image_path: str, min_var: float = 100.0) -> bool:
    img = cv2.imread(image_path, cv2.IMREAD_GRAYSCALE)
    if img is None:
        return False
    var_laplacian = cv2.Laplacian(img, cv2.CV_64F).var()
    return var_laplacian >= min_var

def query_osm_building_heights(lat: float, lon: float, radius: int = 500) -> dict:
    ...
    if response.status_code != 200:
        return {"buildings_count": 0, "max_height_m": 100.0, "elements": []}
    ...
    for el in elements:
        tags = el.get("tags", {})
        h_str = tags.get("height", "0").replace("m", "").strip()
        try:
            h = float(h_str)
        except ValueError:
    ...
    return {
        "buildings_count": len(elements),
        "max_height_m": max_height if max_height > 0 else 120.0,
        "sample_elements": elements[:10]
    }

def main():
    print("🛰️ [CHRONODRIFT] Iniciando Ingesta 6-DoF y Grounding Fáctico...")
    cfg = load_channel_config()
    OUTPUT_DIR.mkdir(parents=True, exist_ok=True)
    
    for ep in cfg.get("episodes_planned", []):
        ep_id = ep["id"]
        city = ep["city"]
        coords_str = ep["coordinates"]
        lat = float(coords_str.split("°")[0].replace("N", "").replace("S", "-").strip())
        lon = float(coords_str.split(",")[1].split("°")[0].replace("E", "").replace("W", "-").strip())
        
        print(f"\n📍 Procesando {ep_id} - {city} ({lat}, {lon})...")
        osm_data = query_osm_building_heights(lat, lon)
        print(f"   🏢 OSM: {osm_data['buildings_count']} edificios detectados. Altura máx: {osm_data['max_height_m']}m")
        
        manifest = {
            "episode_id": ep_id,
            "city": city,
            "coordinates": {"lat": lat, "lon": lon},
            "osm_metrics": osm_data,
            "perspectives_generated": [p["id"] for p in cfg["scraping_6dof_config"]["canonical_perspectives"]]
        }
        
        manifest_file = OUTPUT_DIR / f"{ep_id.lower()}_{city.lower()}_6dof_manifest.json"
        with open(manifest_file, "w", encoding="utf-8") as mf:
            json.dump(manifest, mf, indent=2, ensure_ascii=False)
        print(f"   ✅ Manifiesto 6-DoF guardado: {manifest_file}")
...
Para cada plano del shotlist, se generan previamente **7 keyframes consistentes** que fijan el estado del entorno en intervalos de $0.5\text{ a }1.0\text{ segundos}$.
...
- **Graduación Cromática:** Aplicación uniforme de la paleta de color Kodak Vision3 500T en todos los keyframes del plano.
...
[3_ACTION_MOTION_6DOF] :: Vector cinemático exacto: velocidad (km/h), altitud AGL (m), aceleración ($m/s^2$), trayectorias curvas en Yaw/Pitch/Roll.
...
La estructura de cada episodio de **CHRONODRIFT** (duración total $42.0\text{ a }45.0\text{ segundos}$) se articula en torno a 7 planos canónicos calibrados contra la métrica musical de **118 BPM**:
...
| :- | :--- | :---: | :---: | :---: | :--- | :---: | :---: | :--- |
| **S1** | `01_TERMINAL_DIVE` | $0.0 - 3.05\text{ s}$ | $3.05\text{ s}$ | 1.5 compases | Picado vertical $90^\circ$ atravesando nubes | $850\text{ m} \to 35\text{ m}$ | $140\text{ km/h}$ | **Visual Hook:** Captura instantánea del espectador. Vórtice temporal. |
| **S2** | `02_CANYON_DRIFT` | $3.05 - 10.17\text{ s}$ | $7.12\text{ s}$ | 3.5 compases | Rasante en cañón urbano entre edificios | $1.8\text{ m} \to 15\text{ m}$ | $110\text{ km/h}$ | **Micro-Geografía:** Sensación de escala y textura del suelo. |
| **S3** | `03_TUNNEL_PIERCE` | $10.17 - 18.31\text{ s}$ | $8.14\text{ s}$ | 4.0 compases | Penetración en pasaje estrecho / arcadas | $2.5\text{ m} \to 4.0\text{ m}$ | $85\text{ km/h}$ | **Match-Cut Temporal:** Transición de época 1626 ➔ 2026 dentro de la sombra. |
| **S4** | `04_MONUMENT_ORBIT` | $18.31 - 25.43\text{ s}$ | $7.12\text{ s}$ | 3.5 compases | Órbita $360^\circ$ helicoidal ascendente | $25\text{ m} \to 85\text{ m}$ | $65\text{ km/h}$ | **3D Anchoring:** Hito histórico monumental con cartelería HUD 3D. |
| **S5** | `05_PEDESTRIAN_SWOOP`| $25.43 - 32.55\text{ s}$ | $7.12\text{ s}$ | 3.5 compases | Vuelo a ras de acera a escala humana | $1.5\text{ m} \to 12\text{ m}$ | $45\text{ km/h}$ | **Atmósfera & Transición Futura:** Salto a la biosfera vertical de 2226. |
| **S6** | `06_VERTICAL_SURGE` | $32.55 - 38.65\text{ s}$ | $6.10\text{ s}$ | 3.0 compases | Ascenso vertical trepando megatorre | $12\text{ m} \to 300\text{ m}$ | $125\text{ km/h}$ | **Momentum Vertical:** Aceleración hacia la estratosfera y megacúpulas. |
| **S7** | `07_SKYLINE_SUNSET` | $38.65 - 45.77\text{ s}$ | $7.12\text{ s}$ | 3.5 compases | Gran angular panorámico hacia el horizonte | $300\text{ m} \to 500\text{ m}$ | $90\text{ km/h}$ | **Clímax Tritemporal:** Resolución épica, llamada a la acción y Outro. |

---

### 3.6 Prompts de Producción Completos: EP01 (Tokio)

#### Shot 1: `01_TERMINAL_DIVE` (0.00s - 3.05s)
```
...
#### Shot 2: `02_CANYON_DRIFT` (3.05s - 10.17s)
```
...
#### Shot 3: `03_TUNNEL_PIERCE` (10.17s - 18.31s)
```
...
#### Shot 4: `04_MONUMENT_ORBIT` (18.31s - 25.43s)
```
...
#### Shot 5: `05_PEDESTRIAN_SWOOP` (25.43s - 32.55s)
```
...
#### Shot 6: `06_VERTICAL_SURGE` (32.55s - 38.65s)
```
...
#### Shot 7: `07_SKYLINE_SUNSET_ASCENSION` (38.65s - 45.77s)
```
...
1. **Generación de Voz:** Muestreo a $48\text{ kHz}$ en formato PCM de 24 bits (`es-ES-AlvaroNeural` o voz documental de ElevenLabs).
...
$$\text{Duración de 1 Beat} = \frac{60\,000\text{ ms}}{118} = 508.4746\text{ ms}$$
$$\text{Duración de 1 Compás (4 Beats)} = 4 \times 508.4746\text{ ms} = 2033.898\text{ ms} \approx 2.034\text{ s}$$
$$\text{Frase Musical de 4 Compases} = 4 \times 2033.898\text{ ms} = 8135.593\text{ ms} \approx 8.136\text{ s}$$

Todos los eventos visuales críticos (vórtice de inicio, match-cut de época, revelación de rascacielos) se colocan exactamente en el **Beat 1 de un compás** ($t = n \times 2033.9\text{ ms}$).
...
- **Nivel de Música con Locución:** $-18.0\text{ dB}$ (factor lineal de atenuación: $10^{-18/20} = 0.12589$).
- **Nivel de Música en Clímax Instrumental:** $0.0\text{ dB}$ (ganancia completa).
- **Tiempo de Ataque ($T_{\text{attack}}$):** $30\text{ ms}$ (atenuación instantánea y suave al iniciar la locución).
- **Tiempo de Mantenimiento ($T_{\text{hold}}$):** $50\text{ ms}$ (evita oscilaciones entre palabras cortas).
- **Tiempo de Relajación ($T_{\text{release}}$):** $250\text{ ms}$ (recuperación orgánica de la música al terminar la frase).

```bash
# Filtro FFmpeg Sidechain para Dynamic Ducking de Precisión
ffmpeg -i vo_track.wav -i bgm_118bpm.wav -filter_complex \
"[1:a][0:a]sidechaincompress=threshold=0.08:ratio=6:attack=30:release=250:level_in=1[ducked_bgm]; \
 [ducked_bgm][0:a]amix=inputs=2:weights=1.0 1.0[audio_mix]" \
-c:a pcm_s24le audio_mix.wav
```
...
Con $v_{\text{sonido}} = 343\text{ m/s}$ y un dron a $v_{\text{dron}} = 110\text{ km/h} = 30.55\text{ m/s}$:
- **Aproximación:** $f_{\text{aprox}} = f_0 \left( \frac{343}{343 - 30.55} \right) = f_0 \times 1.0978$ (**$+9.78\%$ de elevación en tono**).
- **Alejamiento:** $f_{\text{alej}} = f_0 \left( \frac{343}{343 + 30.55} \right) = f_0 \times 0.9182$ (**$-8.18\%$ de caída en tono**).
...
- **Braam Drop a $35\text{ Hz}$** con barrido de ruido blanco en fase reversa exactamente en el fotograma del Match-Cut temporal.
...
- **Pico Verdadero Máximo (True Peak):** **$-1.0\text{ dBTP}$** (evita clipping inter-sample en la codificación AAC/Opus de YouTube).
...
ffmpeg -i audio_mix.wav -af \
loudnorm=I=-14.0:TP=-1.0:LRA=7.0:measured_I=-17.8:measured_TP=-0.4:measured_LRA=7.4:measured_thresh=-28.2:offset=0.1:linear=true \
...
La renderización final se ejecuta en **Remotion 4.x** (React + TypeScript + WebGL) con aceleración GPU para mantener $3840 \times 2160$ a 60fps constantes.
...
| **Resolución y Framerate** | $3840 \times 2160$ px @ 60.0 fps exactos | `ffprobe -v error -select_streams v:0` | Re-renderizado forzado en Remotion |
| **Sonoridad Integrada** | $-14.0\text{ LUFS} \pm 0.5\text{ LUFS}$ | `ffmpeg loudnorm (Pass 1 & 2)` | Re-normalización de audio automática |
| **Pico Verdadero (True Peak)** | $\le -1.0\text{ dBTP}$ | `ffmpeg ebur128` | Aplicación de limitador True-Peak |
| **Ducking de Música** | $-18.0\text{ dB}$ atenuación bajo voz | `ffprobe / python sidechain test` | Re-cálculo de envolvente sidechain |
...
## 7. 🚀 Plan Maestro de Despliegue: Los 10 Primeros Episodios
...
| **EP02** | **Nueva York** | 1626 (Nieuw Amsterdam) | 2026 (Manhattan Vertical) | 2226 (Bioluminescent NYC) | MIT Sea-Wall Dikes + Carbon-Graphene Aerodynamics + Columbia Climate School BIG The Big U 2200 |
| **EP03** | **Londres** | 1610 (Tudor Old Bridge) | 2026 (The City & Shard) | 2226 (Sky-Canopy London) | Thames Barrier Phase 4 + Atmospheric Geo-domes + UCL Bartlett School |
| **EP04** | **París** | 1620 (Medieval Île de la Cité) | 2026 (Haussmann & Eiffel) | 2226 (Vertical Forest Paris) | Bioclimatic Thermal Chimneys + Urban Hyperloop Core + Vincent Callebaut Biophilic Paris 2200 |
| **EP05** | **Ámsterdam** | 1626 (Grachtengordel Ring) | 2026 (Canales y Ciclismo) | 2226 (Kinetic Ocean Grid) | TU Delft Hydro-kinetic Floating Foundations + Deltares Sea Level Adaptation |
| **EP06** | **Roma** | 1626 (San Pietro Barroco) | 2026 (Coliseo y Vía Sacra) | 2226 (Subterranean Hyper-Museum) | Geothermal Roman Concrete Regeneration + Sapienza CNR Heritage Climate Resilience |
| **EP07** | **Dubái** | 1833 (Al Fahidi Pearl Village) | 2026 (Burj Khalifa Modern) | 2226 (Cloud-Seeded Geo-Tower) | Solar Desalination Aeroponic Towers + Dubai Future Foundation Desert Arcologies |
| **EP08** | **Hong Kong** | 1841 (Fragrant Harbour) | 2026 (Victoria Peak Neón) | 2226 (Stratospheric Aerodome) | Typhoon Kinetic Dissipation Rings + HKUST Urban Wind Aerodynamics |
| **EP09** | **El Cairo** | 1626 (Ciudadela Mameluca) | 2026 (Guiza y Metrópolis) | 2226 (Nile Solar Arcology) | Desert Carbon Mineralization + Giza Preservation Domes + Nile Resilience Consortium |
| **EP10** | **Venecia** | 1626 (Serenissima Naval) | 2026 (Gran Canal & MOSE) | 2226 (Hydro-Dynamic Crystal) | Ionic Sea-Level Stabilizers + Biomimetic Coral Dikes + Venice Lagoon Preservation Institute |
...
```bash
# 1. Scraping 6-DoF y Grounding Fotogramétrico Completo (10 Ciudades)
...
# 3. Generación por Lotes de Vídeo 4K 60fps en Google Flow
python3 scripts/google_flow_batch_generator.py --manifest data/tritemporal_manifests/tokyo_tritemporal_manifest.json

# 4. Composición Remotion 4.x, Sincronización 118 BPM y Master EBU R128 (-14 LUFS)
python3 scripts/render_remotion.py --city tokyo --target-lufs -14.0 --resolution 4k

# 5. Validación Automatizada de QA Gates
python3 scripts/verify-assets.py --episode EP01
```
