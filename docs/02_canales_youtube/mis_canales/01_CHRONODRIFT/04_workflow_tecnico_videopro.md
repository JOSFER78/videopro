...
│   ├── generate_all_7_keyframes.py         # Generador de 7 keyframes consistentes 4K
...
Cada localización geográfica se descompone en 6 cámaras sintéticas proyectadas a partir de panoramas esféricos equirectangulares de alta resolución (7680 × 3840 px, Master 8K):
...
3. **Copernicus DEM 30m:** Elevación sobre el nivel del mar ($Z_{\text{ground}}$) para calcular la altitud barométrica relativa real ($Z_{\text{rel}} = Z_{\text{dron}} - Z_{\text{ground}}$).
...
graph LR
    ...
    B --> C1[Keyframe 01: Inception 850m]
    B --> C2[Keyframe 02: Cloud Piercing 450m]
    B --> C3[Keyframe 03: Architectural Break 120m]
    B --> C4[Keyframe 04: Temporal Vortex Match-Cut]
    B --> C5[Keyframe 05: Modern Street Orbit]
    B --> C6[Keyframe 06: Future Arcology Surge]
    B --> C7[Keyframe 07: Horizon Climax 500m]
...
| :- | :--- | :---: | :---: | :---: | :--- | :---: | :---: | :--- |
| **S1** | `01_TERMINAL_DIVE` | $0.00 - 3.05\text{ s}$ | $3.05\text{ s}$ | 1.5 compases | Picado vertical $90^\circ$ atravesando nubes | $850\text{ m} \to 35\text{ m}$ | $140\text{ km/h}$ | **Visual Hook:** Captura instantánea del espectador. Vórtice temporal. |
...
```
[# Sources tokyo_s1_kf01.png@Keyframe_Start] [# References tokyo_cam_n.png@Cam_N tokyo_cam_pitchdown.png@Cam_PitchDown]
...
```
[# Sources tokyo_s2_kf01.png@Keyframe_Start] [# References tokyo_cam_e.png@Cam_E tokyo_cam_w.png@Cam_W]
...
```
[# Sources tokyo_s3_kf01.png@Keyframe_Start] [# References tokyo_cam_pitchdown.png@Cam_PitchDown tokyo_cam_s.png@Cam_S]
...
```
[# Sources tokyo_s4_kf01.png@Keyframe_Start] [# References tokyo_cam_n.png@Cam_N tokyo_cam_e.png@Cam_E tokyo_cam_pitchup.png@Cam_PitchUp]
...
```
[# Sources tokyo_s5_kf01.png@Keyframe_Start] [# References tokyo_cam_pitchdown.png@Cam_PitchDown tokyo_cam_w.png@Cam_W]
...
```
[# Sources tokyo_s6_kf01.png@Keyframe_Start] [# References tokyo_cam_pitchup.png@Cam_PitchUp tokyo_cam_n.png@Cam_N]
...
```
[# Sources tokyo_s7_kf01.png@Keyframe_Start] [# References tokyo_cam_pitchdown.png@Cam_PitchDown tokyo_cam_s.png@Cam_S]
...
Para maximizar el algoritmo de recomendación de YouTube Shorts y capturar tráfico masivo móvil, el pipeline implementa una arquitectura de renderizado vertical **9:16 ($2160 \times 3840$ px / $1080 \times 1920$ px)** con diseño de retención hipnótica:
...
3. **Mecánica de Bucle Infinito (Zero-Gap Seamless Loop):**
   - El fotograma final ($t = 32.54\text{ s}$ o $t = 40.67\text{ s}$, coincidiendo exactamente con el compás 16 o 20 a 118 BPM) comparte vector de traslación, velocidad angular y punto focal con el fotograma inicial ($t = 0.00\text{ s}$).
   ...
   - **Bucle Narrativo Abierto:** El guion concluye con una frase puente que enlaza sintáctica y semánticamente con la primera frase del gancho 0-3s, logrando que el usuario promedio consuma el vídeo $1.25\times - 1.45\times$ veces (VTR proyectado $> 120\%$).
...
| :--- | :--- | :--- | :--- |
| **Integridad de Assets** | Todos los archivos $\ge 5.0\text{ KB}$ | `scripts/verify-assets.py` | Aborto de pipeline (`Exit Code 2`) |
...
# 3. Generación y Registro de 7 Keyframes Consistentes 4K por Plano
python3 scripts/generate_all_7_keyframes.py --all

# 4. Pipeline de Audio VO-First, Flow 118 BPM, Foley 3D Doppler y Master EBU R128 (-14 LUFS)
python3 scripts/audio_chronodrift_master.py --all

# 5. Generación por Lotes de Vídeo 4K 60fps en Google Flow
python3 scripts/google_flow_batch_generator.py --manifest data/tritemporal_manifests/tokyo_tritemporal_manifest.json

# 6. Composición Remotion 4.x, Sincronización 118 BPM y Master EBU R128 (-14 LUFS)
python3 scripts/render_remotion.py --city tokyo --target-lufs -14.0 --resolution 4k

# 7. Validación Automatizada End-to-End de QA Gates
